      ...
      if (e.target.files[0]) {
        ...
        createCategoryImage(formData)
          ...
          .then((res) => {
            ...
            setFieldValue(name, { id: res.result.id, path: res.result.path });
            setImage(null);
          })
          .catch((err) => console.log(err));
                ...
                {multiple ? (
                  ...
                  <>
                    {Array.isArray(getIn(values, name)) &&
                      getIn(values, name).map((urlItem, index) => {
                        return (
                          <PreviewImage
                            url={urlItem}
                            // isLoading={isLoading}
...
export default ImageWrapperStateFormik;
